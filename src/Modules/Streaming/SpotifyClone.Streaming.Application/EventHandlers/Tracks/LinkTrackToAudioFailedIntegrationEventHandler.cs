...
internal sealed class LinkTrackToAudioFailedIntegrationEventHandler(
    IBackgroundJobService jobService)
    : INotificationHandler<LinkTrackToAudioFailedIntegrationEvent>
{
    private readonly IBackgroundJobService _jobService = jobService;

    public async Task Handle(
        LinkTrackToAudioFailedIntegrationEvent notification,
        CancellationToken cancellationToken)
        => _jobService.Enqueue<UnlinkAudioAssetFromTrackJob>(job =>
            job.ProcessAsync(notification.AudioId));
}
